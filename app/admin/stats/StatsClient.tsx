        ...
        <div className="overflow-x-auto flex-1">
          <table className="w-full text-left text-sm text-gray-600">
            <thead className="bg-gray-50 text-gray-900 border-b border-gray-200">
              <tr>
                <th className="px-3 py-3 font-semibold w-12 text-center">Rang</th>
                <th className="px-3 py-3 font-semibold">Organisation</th>
                <th className="px-3 py-3 font-semibold">Plan</th>
                <th className="px-3 py-3 font-semibold text-center">Projets</th>
                <th className="px-3 py-3 font-semibold text-center">Membres</th>
                <th className="px-3 py-3 font-semibold text-right">Budget (FCFA)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {topOrgs.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-3 py-6 text-center text-gray-500">Aucune donnée</td>
                </tr>
              ) : topOrgs.map((org, idx) => (
                <tr key={org.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-3 py-3 text-center font-bold text-gray-400">#{idx + 1}</td>
                  <td className="px-3 py-3 font-medium text-gray-900 truncate max-w-[150px]">{org.name}</td>
                  ...
                  <td className="px-3 py-3 text-center font-medium text-gray-900">{org.nb_projets}</td>
                  <td className="px-3 py-3 text-center text-gray-600">{org.nb_membres}</td>
          ...
          <table className="w-full text-left text-sm text-gray-600">
            ...
            <tbody className="divide-y divide-gray-100">
              {recentActivity.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-4 py-6 text-center text-gray-500">Aucune activité récente</td>
                </tr>
                ...
                <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="font-medium text-gray-900 truncate max-w-[150px]">{log.profiles?.full_name || 'Inconnu'}</div>
                    <div className="text-xs text-gray-400 truncate max-w-[150px]">{log.profiles?.email || '-'}</div>
                  </td>
                  <td className="px-4 py-3">
                    <span className="px-2 py-1 bg-gray-100 text-gray-700 rounded-md text-xs font-medium uppercase whitespace-nowrap">
                      {log.module}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-gray-900 text-sm line-clamp-2">{log.action}</span>
                  </td>
                </tr>
              ))}
            </tbody>
