...
export function BudgetClient({ items, fundingSources, operations, objectifsSpecifiques = [], projectId, currency = 'FCFA', isNewProject }: { items: BudgetConsumption[], fundingSources?: any[], operations?: any[], objectifsSpecifiques?: string[], projectId: string, currency?: string, isNewProject?: boolean }) {
  ...
  return (
    <div className="flex flex-col space-y-6">
      ...
      {/* Tableau de Synthèse par Rubrique */}
      {items.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-border overflow-hidden">
          <div className="p-5 border-b border-border bg-surface-dim/30">
            <h3 className="text-lg font-bold text-primary">Résumé par Rubrique</h3>
            <p className="text-sm text-text-secondary">Vue macroscopique de l'exécution budgétaire</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-sm">
              <thead className="bg-primary/5 border-b border-border text-primary font-bold">
                <tr>
                  <th className="p-4 uppercase text-xs tracking-wider">Rubrique</th>
                  <th className="p-4 uppercase text-xs tracking-wider text-right">Budget</th>
                  <th className="p-4 uppercase text-xs tracking-wider text-right">Engagé</th>
                  <th className="p-4 uppercase text-xs tracking-wider text-right">Décaissé</th>
                  <th className="p-4 uppercase text-xs tracking-wider text-right">Solde</th>
                  <th className="p-4 uppercase text-xs tracking-wider w-48">Taux</th>
                  <th className="p-4 uppercase text-xs tracking-wider text-center">Alerte</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border/50">
                {sortedKeys.map(key => {
                  const groupItems = categories[key]
                  const budget = groupItems.reduce((acc, i) => acc + Number(i.initial_allocated_amount), 0)
                  const engage = groupItems.reduce((acc, i) => acc + Number(i.total_engage), 0)
                  const decaisse = groupItems.reduce((acc, i) => acc + Number(i.total_decaisse), 0)
                  const solde = budget - engage // CORRECT LOGIC
                  const taux = budget > 0 ? (engage / budget) * 100 : 0
                  
                  let alerteLabel = 'Normal'
                  let alerteStyle = 'bg-success/10 text-success-dark'
                  if (taux > 90) {
                    alerteLabel = 'Critique'
                    alerteStyle = 'bg-danger/10 text-danger'
                  } else if (taux > 75) {
                    alerteLabel = 'Vigilance'
                    alerteStyle = 'bg-warning/10 text-warning-dark'
                  }
                  ...
                  return (
                    <tr key={key} className="hover:bg-slate-50 transition-colors">
                      <td className="p-4 font-medium text-text-primary">{key}. {objectifName}</td>
                      <td className="p-4 text-right font-mono">{formatCurrency(budget, currency).replace(currency, '')}</td>
                      <td className="p-4 text-right font-mono text-warning-dark">{formatCurrency(engage, currency).replace(currency, '')}</td>
                      <td className="p-4 text-right font-mono text-text-secondary">{formatCurrency(decaisse, currency).replace(currency, '')}</td>
                      <td className="p-4 text-right font-mono font-bold text-primary">{formatCurrency(solde, currency).replace(currency, '')}</td>
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <div className="w-full bg-surface-dim h-2 rounded-full overflow-hidden">
                            <div className="bg-primary h-full rounded-full" style={{ width: `${Math.min(taux, 100)}%` }}></div>
                          </div>
                          <span className="text-xs font-bold w-9">{Math.round(taux)}%</span>
                        </div>
                      </td>
                      <td className="p-4 text-center">
                        <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-bold ${alerteStyle}`}>
                          {alerteLabel}
                        </span>
                      </td>
                    </tr>
